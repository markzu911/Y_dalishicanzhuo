...
import { Sparkles, Home, Table, Sliders, CheckCircle2 } from 'lucide-react';
...
export default function Header({ currentStep, userId, userName, userIntegral }: HeaderProps) {
  const steps = [
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between md:justify-start gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-xl bg-gradient-to-br from-gold-500 to-amber-600 shadow-md shadow-gold-500/10">
              <Sparkles className="w-6 h-6 text-white" />
            </div>
            <div>
              <h1 className="font-display font-semibold text-xl tracking-wide bg-gradient-to-r from-gray-900 via-gray-800 to-gold-600 bg-clip-text text-transparent">
                大理石餐桌智能试摆图生成器
              </h1>
          ...
          {userId && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-50 border border-amber-200 shadow-sm shrink-0 sm:ml-4 w-fit">
              ...
              <div className="h-3 w-[1px] bg-amber-200 mx-1" />
