...
// Global GoogleGenAI instance helper
let aiInstance: GoogleGenAI | null = null;
function getAIInstance() {
  if (!aiInstance) {
    const apiKey = process.env.GEMINI_API_KEY || process.env.API_KEY;
    if (!apiKey) {
      throw new Error("Neither GEMINI_API_KEY nor API_KEY is configured in Vercel environment.");
    }
...
export default async function handler(req: any, res: any) {
  ...
  console.log(`[proxy] Request Method: ${req.method} Path: ${pathname}`);

  // 1. Tool Requests and Uploads Forwarding (/api/tool/* and /api/upload/*)
  if (pathname.startsWith('/api/tool/') || pathname.startsWith('/api/upload/')) {
    const targetUrl = `http://aibigtree.com${pathname}${url.search}`;
    ...
    try {
      const response = await fetch(targetUrl, {
        method: req.method,
        headers: headers,
        body: (req.method === 'POST' || req.method === 'PUT') ? rawBody : undefined,
      });
      ...
      console.error(`[proxy] Error forwarding request to ${targetUrl}:`, proxyError);
      return res.status(502).json({ error: `Bad Gateway: ${proxyError.message}` });
    }
  }
  ...
  // 3. Direct Gemini request (/api/gemini)
  if (pathname === '/api/gemini') {
    ...
    try {
      ...
      console.log(`[proxy] Proxying general request to Gemini with model: ${model}`);
      ...
      console.error("[proxy] Error in /api/gemini:", err);
      return res.status(500).json({ error: err.message || "Failed to process Gemini request" });
    }
  }

  // 4. Room Analysis (/api/analyze-room)
  if (pathname === '/api/analyze-room') {
    if (req.method !== 'POST') {
      return res.status(405).json({ error: "Method Not Allowed" });
    }
    ...
    try {
      ...
      console.log(`[proxy] Analyzing room image...`);
      ...
      console.error("[proxy] Error in /api/analyze-room:", err);
      return res.status(500).json({ error: err.message || "Failed to analyze room image" });
    }
  }

  // 5. Table Analysis (/api/analyze-table)
  if (pathname === '/api/analyze-table') {
    if (req.method !== 'POST') {
      return res.status(405).json({ error: "Method Not Allowed" });
    }
    ...
    try {
      ...
      console.log(`[proxy] Analyzing table image...`);
      const response = await ai.models.generateContent({
        model: "gemini-3.5-flash",
        contents: [
          {
      ...
      console.error("[proxy] Error in /api/analyze-table:", err);
      return res.status(500).json({ error: err.message || "Failed to analyze table image" });
    }
  }

  // 6. Generate Mockup (/api/generate-mockup)
  if (pathname === '/api/generate-mockup') {
    if (req.method !== 'POST') {
      return res.status(405).json({ error: "Method Not Allowed" });
    }
    ...
    try {
      ...
      console.log(`[proxy] Generating mockup image. Size: ${imageSize}, AR: ${ar}`);
      ...
      console.error("[proxy] Error in /api/generate-mockup:", err);
      let errorMessage = err.message || "Failed to generate mockup image";
